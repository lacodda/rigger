...
v0.13.0 makes a sitting a thing the record knows. `session start` opens one and everything written until `session end` belongs to it - the assistant records as it always has, and the boundary is applied by the record rather than guessed from a window of days. `end` says what the sitting held, checks the two things the end-of-session ritual asks for by name, and can append a diary entry composed from the session's own sentences. As an assistant's `Stop` hook it needs no project name: it runs in the project, and the record knows every project by its path. The packet gained the line a returning session actually wants - where the last sitting stopped, and what has happened since. Before it: the database, `import`, the context packet, the `mcp` server, facts from git, a chronicle that writes itself, search, the owner's inbox and digest, the release calendar, the week as a brief and a shopfront queue, and the cycle looked back on. The road to 1.0 is twenty-two small releases in seven blocks:
