...
v0.14.0 turns the record back into notes. `rigger export` writes a project's hub - the plan, the changelog, the diary and the state block of its README - out of the database, and `import` reads them back: the circle closes with every counter at zero. Nineteen of the twenty hubs of a real line are reproduced byte for byte, which is the only way to know that generating them deletes nothing. A hub kept by hand is not overwritten because a command was typed: the first export refuses, `--check` says what would change, `--adopt` hands it over on purpose. `doctor --hubs` names a generated file somebody has edited since, and a project whose hub the record has never seen. The vision, the decision log's prose and the research notes are a person's writing and are never touched. Before it: the database, `import`, the context packet, the `mcp` server, facts from git, a chronicle that writes itself, search, the owner's inbox and digest, the release calendar, the week as a brief and a shopfront queue, the cycle looked back on, and sittings the record knows. The road to 1.0 is twenty-two small releases in seven blocks:
