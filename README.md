...
v0.12.0 closes the owner's block with the look back. `retro` reads a window of the record backwards: how many releases were on time, how many slipped and by how far, how many nobody planned at all - and what share of what shipped had ever been aimed anywhere. It answers the question the written calendar asked every seven weeks and could never perform, because nothing there read a tag. It also says whether the tiers still describe the projects, keeping the two directions apart: a product that has stalled needs a different decision from one that has outgrown its tier. `--record` keeps the summary where a later look back can find it, in a project the record keeps for itself - the first project rigger holds that is not a repository. Before it: the database, `import`, the context packet, the `mcp` server, facts from git, a chronicle that writes itself, search, the owner's inbox and digest, the release calendar, and the week as a brief and a shopfront queue. The road to 1.0 is twenty-two small releases in seven blocks:
