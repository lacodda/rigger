...
fn desk(data: &Path) -> std::path::PathBuf {
    ...
    commit(&repo, "b.txt", "feat(acme-7310): read the rtf header");
    // Another ticket whose id begins with the card's.
    commit(&repo, "c.txt", "fix(ACME-73100): somebody else's ticket");
    git(&repo, &["checkout", "--quiet", "-b", "fix/ACME-7310-rtf"]);
...
fn a_branch_and_a_commit_that_carry_the_key_tie_the_card_to_the_repository() {
    ...
    assert!(out.contains("card       ACME-7310: 1 new commit, 1 branch, linked now"), "{out}");

    let json = show(data.path(), "ACME-7310");
    let links = json["links"].as_array().unwrap();
    assert_eq!(links.len(), 1, "{json:#}");
    assert_eq!(links[0]["project"], "webapp");
    assert_eq!(links[0]["branch"], "fix/ACME-7310-rtf", "the branch found is the link's: {json:#}");

    let activity = &json["activity"][0];
    assert_eq!(activity["project"], "webapp");
    assert_eq!(activity["commits"], 1, "ACME-73100 is another ticket: {json:#}");
    assert_eq!(activity["branches"][0]["name"], "fix/ACME-7310-rtf");
    ...
    let text = output(data.path(), &["task", "show", "ACME-7310"]);
    assert!(text.contains("in git:"), "{text}");
    assert!(text.contains("branch fix/ACME-7310-rtf"), "{text}");
...
fn a_deleted_branch_leaves_the_card_and_its_commits_stay() {
    ...
    git(&repo, &["merge", "--quiet", "--no-edit", "fix/ACME-7310-rtf"]);
    git(&repo, &["branch", "--quiet", "-d", "fix/ACME-7310-rtf"]);
    rigger(data.path()).args(["sync", "webapp"]).assert().success();
    ...
    assert_eq!(json["links"][0]["branch"], "fix/ACME-7310-rtf");
}
...
fn a_branch_only_on_the_remote_says_which_remote() {
    ...
    assert_eq!(cloned["branches"][0]["name"], "fix/ACME-7310-rtf", "{json:#}");
...
fn a_branch_named_by_hand_is_not_overwritten_by_git() {
    ...
    desk(data.path());
    rigger(data.path())
        .args(["task", "link", "ACME-7310", "webapp", "--branch", "hotfix/rtf"])
        .assert()
        .success();
    rigger(data.path()).args(["sync", "webapp"]).assert().success();
    let json = show(data.path(), "ACME-7310");
...
fn an_old_key_kept_as_an_alias_still_finds_the_work() {
    ...
    // old id.
    rigger(data.path()).args(["task", "rename", "ACME-7310", "API-7"]).assert().success();
    let out = output(data.path(), &["sync", "webapp"]);
